...
class DynamicLora(nn.Module):
    """
    FIM-Guided Dynamic LoRA Wrapper.
    
    Instead of hardcoding target_modules, this class accepts dynamically
    determined target layers and rank from FIM sensitivity analysis.
    """
    def __init__(self, args, global_model, target_modules, rank_map):
        super(DynamicLora, self).__init__()
        
        global_model.load_state_dict(
            torch.load('save_model/global_model_{}.pth'.format(args.data_name))
        )
        
        # Compute the median rank from the dynamic rank map
        if rank_map and len(rank_map) > 0:
            ranks = list(rank_map.values())
            median_rank = int(np.median(ranks))
            # Ensure rank is at least 4
            median_rank = max(4, median_rank)
        else:
            median_rank = 16  # fallback default
        
        print(f"[DynamicLora] Target modules: {target_modules}")
        print(f"[DynamicLora] Rank map: {rank_map}")
        print(f"[DynamicLora] Using median rank: {median_rank}")
        
        config = LoraConfig(
            r=median_rank,
            lora_alpha=median_rank * 2,  # standard 2x scaling
            target_modules=target_modules,
            lora_dropout=0.1,
            bias="none",
        )
        ...
        # Freeze all parameters except the LoRA adapter weights
        for name, param in self.lora_model.named_parameters():
        ...
        print(f"[DynamicLora] Total params: {total_params:,}, Trainable (LoRA): {trainable_params:,} ({100*trainable_params/total_params:.2f}%)")
